...
`leaderboard` rows carry `MetricsSchema` (the challenge's declared metric columns) and `MeanMetricsDetail` (the precomputed per-metric aggregate) — passthrough columns in the returned DataFrame, no client code needed. `update_settings` accepts `evaluation_metrics_schema` (ordered descriptor list) + `evaluation_frontend_precision` to author them; `source:"env"` descriptor keys are exactly what `env.evaluate` must return in `metrics_detail`.
